...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public void init(Context context, AttributeSet attrs) {
        ...
        if (!format.isEmpty()){
            mHeaderLayout.setLastUpdatedLabel(format);
        }
    ...
    protected void refreshRefreshableViewSize(int width, int height) {
        if (null != mRefreshableViewWrapper) {
            ...
            if (lp.height != height) {
                lp.height = height;
                //LogUtil.d("refreshRefreshableViewSize  height = " + height);
     ...
     * 当状态发生变化时调用
     *
     * @param ILoadingLayout.State 状态
     * @param isPullDown 是否向下
     */
    protected void onStateChanged(ILoadingLayout.State ILoadingLayout, boolean isPullDown) {
